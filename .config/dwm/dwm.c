...
{
  int x, w, tw = 0, stw = 0;
  int boxs = drw->fonts->h - 1;     // /9
  int boxw = drw->fonts->h / 6 + 2; // /6 + 2
  ...
  if (!systray)
    {
      ...
      wa.colormap = cmap;
      systray->win
          = XCreateWindow (dpy, root, x - xpad, m->by + ypad, w, bh, 0, depth,
                           InputOutput, visual,
                           CWOverrideRedirect | CWBackPixel | CWBorderPixel
                               | CWColormap | CWEventMask,
                           &wa);
      ...
      XChangeProperty (dpy, systray->win, netatom[NetWMWindowType], XA_ATOM,
                       32, PropModeReplace,
